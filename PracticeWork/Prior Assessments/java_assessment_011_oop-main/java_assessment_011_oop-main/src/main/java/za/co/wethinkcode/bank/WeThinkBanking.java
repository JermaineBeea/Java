...
public class WeThinkBanking {
    ...
    private void doDepositMenu() {
        ...
        account.creditAccount(new Money((int) depositAmount));
    ...
    private void doWithdrawMenu() {
        ...
        int withdrawAmount = (int) io.anyFloat("How much would you like to withdraw? ");
        Account account = listOfAccounts.get(inputValidated-1);
        account.debitAccount(new Money(withdrawAmount));
        printAccountBalance(account);
    }
    ...
    private void openNewAccount(String accountName, String accountType) {
        if (accountType.contains("savings")) {
            ...
            addAccount(accountName.isBlank() ? new BusinessAccount() : new BusinessAccount(accountName, null));
        } else {
    ...
    private String formatAccountOutput(Account account) {
        return account.getAvailableBalance().formattedAsRands();
    }
}
